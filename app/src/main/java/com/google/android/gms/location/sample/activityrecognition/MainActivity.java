...
public class MainActivity extends ActionBarActivity implements
    ...
    private LineGraphSeries<DataPoint> seriesFoot;
    private LineGraphSeries<DataPoint> seriesStill;
    private LineGraphSeries<DataPoint> seriesVehicle;
    private LineGraphSeries<DataPoint> seriesRunning;
//    private LineGraphSeries<DataPoint> seriesTilt;
    private LineGraphSeries<DataPoint> seriesBicycle;
    private LineGraphSeries<DataPoint> seriesWalking;
//    private LineGraphSeries<DataPoint> seriesUnknown;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        seriesFoot = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesBicycle = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesRunning = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesStill = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        /*seriesTilt = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesUnknown = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });*/
        seriesVehicle = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesWalking = new LineGraphSeries<>(new DataPoint[]{
                new DataPoint(0, 0)
        });
        seriesWalking.setColor(Color.RED);
        seriesVehicle.setColor(Color.BLACK);
        /*seriesUnknown.setColor(Color.BLUE);
        seriesTilt.setColor(Color.YELLOW);*/
        seriesBicycle.setColor(Color.CYAN);
        seriesFoot.setColor(Color.GREEN);
        seriesRunning.setColor(Color.MAGENTA);
        seriesStill.setColor(Color.BLUE);
        ...
        graphView.addSeries(seriesFoot);
        graphView.addSeries(seriesBicycle);
        graphView.addSeries(seriesRunning);
        graphView.addSeries(seriesStill);
        /*graphView.addSeries(seriesUnknown);
        graphView.addSeries(seriesTilt);*/
        graphView.addSeries(seriesVehicle);
        graphView.addSeries(seriesWalking);
    ...
    protected void updateActivities(ArrayList<DetectedActivity> detectedActivities) {
        ...
        for (DetectedActivity detectedActivity : tempList) {
            if (detectedActivity.getType() == DetectedActivity.ON_FOOT) {
                seriesFoot.appendData(new DataPoint((int) ++point, detectedActivity.getConfidence()), true, 100);
            }
            if (detectedActivity.getType() == DetectedActivity.ON_BICYCLE) {
                seriesBicycle.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }
            if (detectedActivity.getType() == DetectedActivity.IN_VEHICLE) {
                seriesVehicle.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }
            if (detectedActivity.getType() == DetectedActivity.STILL) {
                seriesStill.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }
            /*if (detectedActivity.getType() == DetectedActivity.TILTING) {
                seriesTilt.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }*/
            if (detectedActivity.getType() == DetectedActivity.WALKING) {
                seriesWalking.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }
            if (detectedActivity.getType() == DetectedActivity.RUNNING) {
                seriesRunning.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }
            /*if (detectedActivity.getType() == DetectedActivity.UNKNOWN) {
                seriesUnknown.appendData(new DataPoint((int) point, detectedActivity.getConfidence()), true, 100);
            }*/
        }
        seriesVehicle.setOnDataPointTapListener(new OnDataPointTapListener() {
            @Override
            public void onTap(Series series, DataPointInterface dataPoint) {
                Toast.makeText(MainActivity.this, "Vehicle: "+dataPoint, Toast.LENGTH_SHORT).show();
            }
        });
        seriesBicycle.setOnDataPointTapListener(new OnDataPointTapListener() {
            @Override
            public void onTap(Series series, DataPointInterface dataPoint) {
                Toast.makeText(MainActivity.this, "Bicycle: "+dataPoint, Toast.LENGTH_SHORT).show();
            }
        });
        seriesFoot.setOnDataPointTapListener(new OnDataPointTapListener() {
            @Override
            public void onTap(Series series, DataPointInterface dataPoint) {
                Toast.makeText(MainActivity.this, "Foot: "+dataPoint, Toast.LENGTH_SHORT).show();
            }
        });
        seriesWalking.setOnDataPointTapListener(new OnDataPointTapListener() {
            @Override
            public void onTap(Series series, DataPointInterface dataPoint) {
                Toast.makeText(MainActivity.this, "Walking: "+dataPoint, Toast.LENGTH_SHORT).show();
            }
        });
    }
}
